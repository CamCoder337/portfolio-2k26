 ...
 *
 * The `www` host is the one Vercel actually serves; the apex redirects to it.
 * This has to match, or every canonical points at a URL that redirects and the
 * browser's own origin stops matching the Sanity CORS allowlist.
 */
const url = "https://www.cam-coder.com";
...
export const site = {
  ...
  url,
  /**
   * The host as a person would say it, without the scheme or the `www`. Only
   * for display — the social cards print it. Anything a machine resolves uses
   * `url`, which has to stay the exact origin being served.
   */
